<script>
    ...
    $effect(() => {
        ...
        // Use JSON.parse(JSON.stringify()) to unwrap Proxy objects in production builds
        const plainCategories = JSON.parse(JSON.stringify(data.categories));
        const sortedCategories = plainCategories.sort(
            (a, b) => b.count - a.count,
        );
